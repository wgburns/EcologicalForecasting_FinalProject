...
#### 3.1.2 2018 Cyanobacteria bloom development 

[haven't done this for 2018 yet]

### 3.2 Feature importance 

Feature importance with both bays and years combined suggests that discharge, solar radiation, and specific conductivity were the most important features for explaining contemporaneous PC levels, while air temperature, solar radiation and wind speed were most important for future PC levels (Fig. 4). Year was an important feature, while Bay was not.
...
#Fix Date Format
db2 <- db %>% 
  select(-c(X, pHMean, TempMean, ODOMean, ChlMean, TurbMean, DeltaDOMean, PCMean2:PCMean7)) %>% 
  ...
  gather(key = "parameter", value = "value", 
         c(PC, DeltaTemp, Discharge, Solar, SpCond, AirTemp, wd, ws))  

#reordering the parameters so when i plot it matches the colors of the stacked bar graphs 
db2$parameter <- factor(db2$parameter, c("PC", "DeltaTemp", "Discharge", "Solar", "SpCond", "AirTemp", "wd", "ws"))
...
db2 %>% 
  filter(bay == "MB") %>% 
  ggplot(aes(x = doy, y = value, group = year, color = parameter)) +
