...
data <- read.csv("/Users/chege/Desktop/quantitative reasoning final project/HF_stAB18_Clean.csv")
...
bloom_variable <-data%>%
  select(PC_RFU_0.5m) # get the column with the bloom measurements


# check out the first few lines
head(bloom_variable) # of our target variable

#Just have a dataframe with the target variable removed
...
without_bloom_numeric <- without_bloom_indicator %>%
  ...
  select(-c(PH_0.5m,ID,Turb_NTU_0.5m,  TIMESTAMP,CHL_RFU_0.5m, deltaDO)) %>% # Here i have removed Ph and timestamp
