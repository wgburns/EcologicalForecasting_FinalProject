...
## Prettier Time Plots with ggPlot2?

```{r}
ggplot(MB17_daily, aes( x = date, y = PCMean1)) + geom_line(size = 0.75) + labs(title = " PC over Time") + xlab("Time") + ylab("PC") + geom_line(data = MB17_daily, aes(x = date, y = DeltaTempMean), color = "red") + geom_line(data = MB17_daily, aes(x = date, y = SpCondMean), color = "green")
```
Y-axes are too different. Better to have separate plots.
